...
public class ShootScript : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))    //Left mouse button
        {
            ...
            try
            {
                _offset = this.transform.position + 1.5f * this.transform.forward;
                _gObjList.Add(Instantiate(Bullet, _offset, Quaternion.LookRotation(this.transform.forward, Vector3.up)));
